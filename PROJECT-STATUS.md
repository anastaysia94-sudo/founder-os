...
A source audit found two browser-state hazards before genuine account-switch acceptance:

1. an old asynchronous hydration request could theoretically finish after sign-out or a user switch and repaint stale private Business Record state;
2. during a direct A → B account switch, user A's existing record could remain visible until user B's hydration completed.

The accepted source now:

- version-tags hydration requests and ignores superseded results;
- rejects hydration belonging to a user who is no longer active;
- clears private Business Record state on sign-out;
- clears the previous account's private UI state before hydrating a newly active account;
...
The accepted runtime uses Next.js 16.3.4. The CI dependency high/critical vulnerability gate passes for the current account-switch-safe revision.
...
- Railway `/api/health`: succeeded on the first observed healthcheck attempt
...
- accepted account-switch-safe web revision is deployed
...
- previous-account private state is cleared before a new account hydrates
